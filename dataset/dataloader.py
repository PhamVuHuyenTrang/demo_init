...
def get_malware_dataset(valid=False):
    ...
    def read_data(valid, folder_name='./data/malware-dataset/'):
        if valid:
            # first month
            xs, ys = get_monthly_data('./data/malware-dataset/2010-11.txt')
        else:
            ...
            xs, ys = [], []
            for f in file_names:
                x_mat, y_vec = get_monthly_data('./data/malware-dataset/' + f)
                xs.append(x_mat)
                ys.append(y_vec)
...
def get_elec2_dataset(valid=False):
    ...
    def get_all_data(file_path='./data/electricity-price-dataset/electricity-normalized.csv', 
                     num_feature=15):
...
def get_sensordrift_dataset(valid=False):
    ...
    class_id = 2
    gas_class =  get_batch_data('./dataset/sensor-drift-dataset/batch1.dat') # validation
    ...
    if valid:
        ...
        for file_name in file_names:
            gas_class = get_batch_data('./dataset/sensor-drift-dataset/'+file_name)
